...
class PlotTriangles: public EnvironmentObject {
public:

	typedef boost::shared_ptr<PlotTriangles> Ptr;
	osg::ref_ptr<osg::Geode> m_geode;
	...
	// adds a triangle.
	void addTriangle(const osg::ref_ptr<osg::Vec3Array>& vertices, const osg::ref_ptr<osg::Vec4Array>& colors);
	void addTriangle(const osg::ref_ptr<osg::Vec3Array>& vertices);
	void addTriangle(const vector3f &vertices, const vector4f &colors);
	void addTriangle(const vector3f &vertices);
};
