...
void PlotPoints::setPoints(const vector3f & pts, const vector4f & cols) {
  setPoints(toVec3Array(pts), toVec4Array(cols));
}
void PlotPoints::setPoints(const vector3f & pts) {
...
void PlotLines::setPoints(const vector3f& pts, const vector4f& cols) {
  setPoints(toVec3Array(pts),  toVec4Array(cols));
}

void PlotLines::setPoints(const vector3f & pts) {
...
void PlotCurve::setPoints(const vector3f & pts, const vector4f & cols) {
  setPoints(toVec3Array(pts),  toVec4Array(cols));
}

void PlotCurve::setPoints(const vector3f & pts) {
  osg::ref_ptr<osg::Vec4Array> osgCols = new osg::Vec4Array(pts.size());
  BOOST_FOREACH(osg::Vec4& col, *osgCols) col = m_defaultColor;
  setPoints(toVec3Array(pts),  osgCols);
}
...
void PlotPolygons::addPolygon(const vector3f& pts, const vector4f& cols) {
  addPolygon(toVec3Array(pts), toVec4Array(cols));
}

void PlotPolygons::addPolygon(const vector3f& pts) {
...
void PlotTriangles::setDefaultColor(float r, float g, float b, float a) {
  m_defaultColor = osg::Vec4(r,g,b,a);
}

// adds a polygon. Assumes that the vertices are given in counter-clockwise manner.
void PlotTriangles::addTriangle(const osg::ref_ptr<osg::Vec3Array>& vertices, const osg::ref_ptr<osg::Vec4Array>& colors) {
	int nPts = vertices->getNumElements();
	osg::Geometry* m_geom = new osg::Geometry();
	m_geom->setColorArray(colors);
	m_geom->setVertexArray(vertices);
	m_geom->setColorBinding(osg::Geometry::BIND_PER_VERTEX);
	m_geom->getPrimitiveSetList().clear();
	m_geom->addPrimitiveSet(new osg::DrawArrays(osg::PrimitiveSet::TRIANGLES,0,nPts));
...
void PlotTriangles::addTriangle(const osg::ref_ptr<osg::Vec3Array>& vertices) {
  osg::ref_ptr<osg::Vec4Array> osgCols = new osg::Vec4Array(vertices->size());
  BOOST_FOREACH(osg::Vec4& col, *osgCols) col = m_defaultColor;
  addTriangle(vertices, osgCols);
}


void PlotTriangles::addTriangle(const vector3f& pts, const vector4f& cols) {
	addTriangle(toVec3Array(pts), toVec4Array(cols));
}

void PlotTriangles::addTriangle(const vector3f& pts) {
	addTriangle(toVec3Array(pts));
}
